	...
	public class ReviewDAO {
		...
		public ArrayList<ReviewVO> getPostingList(PagingBean pagingBean) throws SQLException{
			...
			try{
				...
				while(rs.next()){		
					...
					rvo.setReviewGrade(rs.getDouble(9));
		...
		public int registerReview(ReviewVO review) throws SQLException {
			...
			try {
				...
				pstmt.setDouble(3, review.getReviewGrade());
		...
		public int getItemNamePostCount(String itemName) throws SQLException {
			int totalCount=0;
			Connection con=null;
			PreparedStatement pstmt=null;
			ResultSet rs=null;
			try {
				...
				String sql="SELECT count(*)\r\n" + 
						...
						"as rnum,review_no,review_title,to_char(review_regdate,'YYYY.MM.DD')\r\n" + 
						",review_hit FROM review) rn, review r, member m, item i\r\n" + 
						"WHERE r.id=m.id and r.item_no=i.item_no and rn.review_no=r.review_no and i.item_name=?";
				pstmt=con.prepareStatement(sql);
		...
		public ArrayList<ReviewVO> getPostingMyItemNameList(PagingBean pagingBean, String itemName) throws SQLException {
			ArrayList<ReviewVO> list=new ArrayList<ReviewVO>();
			Connection con=null;
			PreparedStatement pstmt=null;
			ResultSet rs=null;
			try{
				...
				sql.append("SELECT r.review_no,r.review_title,to_char(review_regdate,'YYYY.MM.DD')\r\n" + 
						...
						"as rnum,review_no,review_title,to_char(review_regdate,'YYYY.MM.DD')\r\n" + 
						",review_hit FROM review) rn, review r, member m, item i\r\n" + 
						"WHERE r.id=m.id and r.item_no=i.item_no and rn.review_no=r.review_no and i.item_name=? AND\r\n" + 
						"rnum BETWEEN ? AND ? ORDER BY r.review_no DESC");
				pstmt=con.prepareStatement(sql.toString());	
				pstmt.setString(1, itemName);
				pstmt.setInt(2, pagingBean.getStartRowNumber());
