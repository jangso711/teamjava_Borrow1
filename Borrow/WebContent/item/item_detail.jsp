		...
		<table class="table">
			...
			<tbody>
				<tr>
					...
					<td>
						<!-- 180903 MIRI ItemDAO에서 처리 --> <%-- <!-- 180901 MIRI 사진이 없으면 디폴트 이미지 띄움 -->
							<c:if test="${empty itemDetail.picList }">
								<img src="${pageContext.request.contextPath }/upload/디폴트.png">
							</c:if> --%> <!-- 180901 MIRI 상품 전체 사진 리스트를 불러와 사진이 있으면 사진을 띄움 -->
					...
					<td>
						<!-- 180901 MIRI 상품 전체 카테고리 리스트를 불러옴 --> <c:forEach
							items="${itemDetail.catList }" var="catList">
							${catList.catName }<br>
						</c:forEach>
					</td>
					<td>${item.itemName }</td>
				...
				<c:otherwise>
					<body>
						<form action="front" method="post">
							...
							대여날짜 입력<input id="from" type="text" value=""
								name="rentalDate" min="${item.itemRegDate}"
								max="${item.itemExpDate}" required="required"><br>
							반납날짜 입력<input id="to" type="text" value=""
								name="returnDate" min="${item.itemRegDate}"
								max="${item.itemExpDate}" required="required"><br>
